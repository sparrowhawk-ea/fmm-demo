(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.FmmDemo = {}));
	...
	const _Ea = class {
	  constructor() {
	    ...
	    this.useNamesShown = [];
	    this.namesRemaining = [...Object.keys(_Ea.namesAll)];
	    ...
	    this.randomQuotes = _Ea.randomize(Object.entries(_Ea.quotes), 5);
	  }
	  static randomize(input, count) {
	    ...
	    return count === void 0 ? input : input.slice(0, count);
	  }
	  addRandomUseName() {
	  ...
	  removeUncheckedUseNames() {
	    ...
	    for (let i = this.useNamesShown.length; --i >= 0; ) {
	      ...
	      namesRemoved.push(_Ea.namesAll[useName]);
	  ...
	  updateRealNamesShown() {
	    const selected = this.useNamesGetSelected();
	    const checkedNames = [];
	    this.useNamesShown.forEach((n, i) => selected[i] && checkedNames.push(_Ea.namesAll[n]));
	...
	Ea.aggregateLabels = {
	  quoteRadios: _Ea.controls.quoteRadios.label,
	  useNames: _Ea.controls.useNames.label,
	  useNamesAll: _Ea.controls.useNamesAll.label
	};
